...
class Context : private UniformProvider {
 ...
 private:
  ...
  std::unordered_map<std::string, std::vector<RenderUnitPtr>> renderUnits;

 private:
  Context &operator=(Context &ctx);

 public:
  void add(Pass const &p) { passes.push_back(p); }
  void add(std::shared_ptr<Renderable> const r) {
    ...
    for (auto renderUnit : r->getUnits()) {
      auto passOpts =
          std::find_if(passes.begin(), passes.end(), [&renderUnit](Pass &p) {
  ...
  std::vector<RenderUnitPtr> getRenderUnits(std::string forPass) const {
